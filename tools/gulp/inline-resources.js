...
function inlineResources(projectPath) {
  ...
  // Match only scss files in projectPath
  const scssFiles = glob.sync('./components/**/*.scss', { cwd: projectPath });

  // Match only TypeScript files in projectPath.
  const tsFiles = glob.sync('**/*.ts', { cwd: projectPath });

  return Promise.resolve()
  .then(() => {
    // For each file, fix the scss import statement and write the new file
    return Promise.all(
      scssFiles.map(filePath => {
        ...
        .then(content => {
          return content.replace(/@import "~scss\/(.*?)"/g, function(line, group) {
            return `@import "${path.relative(path.dirname(fullFilePath), projectPath).replace(/\\/g, '/')}/${group}"`;
          });
        })
        .then(content => {
